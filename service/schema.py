...
class GenerateInvoice(graphene.Mutation):
    ...
    def mutate(cls, root, info, request_id, total_amount, parts_replaced=None, notes=None):
        ...
        # No status condition — pick the latest estimation regardless of status
        estimation = repair_request.estimations.order_by("-created_at").first()
        ...
        pdf_url = create_invoice_pdf(invoice, [estimation] if estimation else [])
        invoice.pdf_url = pdf_url
        invoice.save(update_fields=["pdf_url"])
